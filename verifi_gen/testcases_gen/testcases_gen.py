...
print('uvm testcases generator 1.0')
print('')

# set template path
template_path = os.getcwd() + os.sep + 'template_testcases'
...
testcases_dir = env_name + '_testcases'
if os.path.exists(testcases_dir):
    shutil.rmtree(testcases_dir)
os.mkdir(testcases_dir)
print('RESULT DIR: ' + testcases_dir)
print('')

# define template handle function
def uvm_file_gen(filename, **kwargs):
    uvm_template = mylookup.get_template(filename)
    fd = open(testcases_dir + os.sep + filename.replace('template', env_name), 'w',newline='')
    fd.write(uvm_template.render(**kwargs))
    fd.close()
    print('Generating ' + testcases_dir + os.sep + filename.replace('template', env_name))

# template rendering
uvm_file_gen('template_basic_testcase.svh', env_name=env_name, basic_test_name=basic_test_name, vsqr_name=vsqr_name, interface=interface,
             agent=agent, reg_model_name=reg_model_name, scoreboard=scoreboard, coverage=coverage, seq_item=seq_item)
uvm_file_gen('template_testcases_pkg.sv', env_name=env_name, basic_test_name=basic_test_name, vsqr_name=vsqr_name, interface=interface,
             agent=agent, reg_model_name=reg_model_name, scoreboard=scoreboard, coverage=coverage, seq_item=seq_item)